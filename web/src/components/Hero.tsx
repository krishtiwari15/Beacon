    ...
    <section className="relative min-h-screen w-full overflow-hidden sm:h-screen">
      ...
      <div className="relative z-10 flex flex-col items-center px-4 pt-24 text-center sm:px-6 sm:pt-28 md:pt-32">
        <Animate delay={200} direction="up">
          <h1 className="max-w-5xl font-serif text-[2rem] leading-[1.05] font-medium tracking-tight text-[var(--heading)] sm:text-4xl md:text-5xl lg:text-[4.75rem] xl:text-[5.25rem]">
            Track every opportunity{" "}
            <span className="text-[var(--heading-accent)] italic">
              before the deadline
    ...
    <>
      <nav className="absolute top-0 right-0 left-0 z-30 flex items-center justify-between px-4 py-4 sm:px-6 sm:py-6 md:px-10">
        <Animate delay={0} direction="down">
          <span className="font-serif text-lg font-semibold tracking-tight text-[var(--heading)] sm:text-xl md:text-2xl">
            Beacon
          </span>
        </Animate>
          ...
          <div className="flex flex-col gap-1">
            {NAV_LINKS.map((link, i) => (
              <button
                ...
                className={`border-b border-[var(--text)]/10 py-4 text-left font-serif text-2xl font-medium text-[var(--text)] transition-all duration-500 ${
